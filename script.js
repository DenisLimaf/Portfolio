const modalData = {
    projetos: {
        ...
        icon: '💻',
        link: 'projetos.html',
        content: 'Além disso, fiz cursos idiomáticos (Inglês) e cursos de programação.'
    },
    ...
    habilidades: {
        ...
        title: 'Habilidades',
        icon: '⚡',
        link: 'habilidades.html',
        content: 'Domínio em Administração, Inglês e Programação, com conhecimentos em tecnologias fundamentais da web, desenvolvimento de páginas e lógica de programação. Possuo facilidade para aprender novas ferramentas e me adaptar a diferentes situações, buscando constantemente aprimorar meus conhecimentos e desenvolver novas habilidades.'
    },
    ...
    contato: {
        ...
        icon: '🎯',
        link: 'contato.html',
        content: 'Estou em busca de novas oportunidades na área de programação, especialmente em vagas júnior, projetos freelancer e parcerias. Meu objetivo é desenvolver minhas habilidades na prática, adquirir experiência profissional e contribuir para a criação de soluções eficientes e de qualidade.'
    }
};
...
// Inicializa os event listeners diretamente
...
if (modalIcon) {
    modalIcon.addEventListener('mouseenter', () => {
        ...
        if (hoverCount === 21) {
            if (currentModalKey === 'projetos') {
                startMatrixEffect();
            } else if (currentModalKey === 'habilidades') {
                startTechEffect();
            } else if (currentModalKey === 'contato') {
                startContactEffect();
            }
        }
    });
...
function openModal(key) {
    ...
    document.getElementById('modal-title').href = data.link;
...
function stopEffects() {
    ...
    if (modalIcon && currentModalKey === 'contato') {
...
function startTechEffect() {
    ...
    function spawnText() {
        const text =
            techList[Math.floor(Math.random() * techList.length)];
        ...
        if (hackPhase === 'floating') {
            ...
            if (Math.random() < 0.015 && hackingLogs.length < 25) {
                ...
                const extraLogs = [
                    ...
                    '>> PARSING LOCAL STACK PACKETS...',
                    '>> CRACKING ROOT PRIVILEGES...',
                    '>> BYPASSING PORTFOLIO AUTH...'
                ];


                hackingLogs.push(
                    extraLogs[
                        Math.floor(Math.random() * extraLogs.length)
                    ]
                );


                if (hackingLogs.length > 20) {
                    hackingLogs.shift();
                }
            }
            ...
            for (let i = particles.length - 1; i >= 0; i--) {
                ...
                if (
                    p.frameCounter % 15 === 0 &&
                    p.progress < p.originalText.length
                ) {
                    p.progress++;
                }


                p.displayText = getEncryptedText(
                    p.originalText,
                    p.progress
                );
                ...
                ctx.font =
                    `bold ${p.fontSize}px Courier New, monospace`;


                const textWidth =
                    ctx.measureText(p.displayText).width;

                const textHeight = p.fontSize;


                const dist = Math.hypot(
                    (p.x + textWidth / 2) - mouseX,
                    (p.y - textHeight / 2) - mouseY
                );
            ...
            for (let i = sparks.length - 1; i >= 0; i--) {
                ...
                ctx.font =
                    `${s.fontSize}px Courier New, monospace`;
            ...
            // Transition to Loading Phase after 10 seconds
            if (elapsed >= 10000) {
